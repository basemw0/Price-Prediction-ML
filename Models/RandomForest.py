...
X1encoder  = OneHotEncoder()
X1OneHotFit = X1encoder.fit(data.X1_new.values.reshape(-1, 1))
X1OneHot = X1encoder.transform(data.X1_new.values.reshape(-1, 1)).toarray()
dfOneHot = pd.DataFrame(X1OneHot, columns=["X1_" + str(int(i)) for i in range(X1OneHot.shape[1])])
data = pd.concat([data, dfOneHot], axis=1)
data = data.drop(["X1","X1_new"] ,axis=1)
data2 = data2.drop(["X1"],axis=1)

# Drop unnecessary columns
data = data.drop(["X2","X7_1","X7_2","X7_3","X7_6","X7_7","X7_8","X7_9","X9","X11_2","X7_5","X11_0","X4"], axis=1)
data2 = data2.drop(["X2","X7_1","X7_2","X7_3","X7_6","X7_7","X7_8","X7_9","X9","X11_2","X7_5","X11_0","X4"], axis=1)
...
data = data.drop(["X1_0","X1_1","X1_2"], axis=1)
...
print("\nStarting grid search with Random Forest...")
param_grid_rf = {
...
print("Best CV score (RMSE):", np.sqrt(-grid_search_rf.best_score_))
...
print(f"Min: {rf_predictions.min():.3f}")
print(f"Max: {rf_predictions.max():.3f}")
print(f"Mean: {rf_predictions.mean():.3f}")
print(f"Std: {rf_predictions.std():.3f}")
